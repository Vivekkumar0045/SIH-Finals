...
# Custom video transformer for capturing video frames
        ...
        if len(data_aux) == 84:  # If the hand landmarks are ready
            ...
            if confidence >= confidence_threshold and predicted_label in labels_dict:
                ...
                if self.consecutive_frames >= 2:
                    if not self.prediction_sequence or self.prediction_sequence[-1] != predicted_character:
                        self.prediction_sequence.append(predicted_character)

                        if len(self.prediction_sequence) >= 2:
                            last_two = tuple(self.prediction_sequence[-2:])
...
# Run the webrtc streamer to display video
webrtc_streamer(key="hand-gesture-recognition", video_transformer_factory=VideoTransformer)
